...
import {AddressToBytes32Lib} from "lib/boring-vault/src/helper/AddressToBytes32Lib.sol";
import {ChainValues} from "lib/boring-vault/test/resources/ChainValues.sol";
import {Strings} from "lib/boring-vault/lib/openzeppelin-contracts/contracts/utils/Strings.sol";
import {ERC4626} from "lib/boring-vault/lib/solmate/src/tokens/ERC4626.sol";
import {Address} from "lib/boring-vault/lib/openzeppelin-contracts/contracts/utils/Address.sol";
import {IComet} from "lib/boring-vault/src/interfaces/IComet.sol";
import {TellerWithMultiAssetSupport} from "lib/boring-vault/src/base/Roles/TellerWithMultiAssetSupport.sol";
import {BaseDecoderAndSanitizer} from "lib/boring-vault/src/base/DecodersAndSanitizers/BaseDecoderAndSanitizer.sol";
...
contract MerkleTreeHelper is CommonBase, ChainValues {
    ...
    function _addLeafsFor1InchGeneralSwapping(
        ...
        for (uint256 i; i < assets.length; ++i) {
            ...
            if (
                !tokenToSpenderToApprovalInTree[assets[i]][
            ...
            for (uint256 j; j < assets.length; ++j) {
                ...
                if (
                    !oneInchSellTokenToBuyTokenToInTree[assets[i]][assets[j]] &&
                ...
                if (
                    ...
                    !oneInchSellTokenToBuyTokenToInTree[assets[j]][assets[i]]
                ) {
                    // Add buy swap.
                    unchecked {
                        leafIndex++;
    ...
    function _addLeafsFor1InchUniswapV3Swapping(ManageLeaf[] memory leafs, address pool) internal {
        ...
        // Add approval leaf if not already added
        if (
            !tokenToSpenderToApprovalInTree[token0][getAddress(sourceChain, "aggregationRouterV5")]
        ) {
            unchecked {
                leafIndex++;
            }
            ...
            tokenToSpenderToApprovalInTree[token0][
                getAddress(sourceChain, "aggregationRouterV5")
            ] = true;
        }
        if (
            !tokenToSpenderToApprovalInTree[token1][getAddress(sourceChain, "aggregationRouterV5")]
        ) {
            unchecked {
                leafIndex++;
            }
            ...
            leafs[leafIndex].argumentAddresses[0] = getAddress(sourceChain, "aggregationRouterV5");
            tokenToSpenderToApprovalInTree[token1][
                getAddress(sourceChain, "aggregationRouterV5")
            ] = true;
        }
    ...
    function _addConvexLeafs(
        ...
        if (
            !tokenToSpenderToApprovalInTree[address(token)][
    ...
    function _addCcipBridgeLeafs(
        ...
        for (uint256 i; i < feeTokens.length; i++) {
            if (
                !tokenToSpenderToApprovalInTree[address(feeTokens[i])][
            ...
            for (uint256 j; j < bridgeAssets.length; j++) {
                if (
                    !tokenToSpenderToApprovalInTree[address(bridgeAssets[j])][
                        getAddress(sourceChain, "ccipRouter")
                    ]
                ) {
    ...
    function _addPancakeSwapV3Leafs(
        ...
        for (uint256 i; i < token0.length; ++i) {
            ...
            // Approvals
            if (
                !tokenToSpenderToApprovalInTree[token0[i]][
            ...
            if (
                !tokenToSpenderToApprovalInTree[token1[i]][
                    getAddress(sourceChain, "pancakeSwapV3NonFungiblePositionManager")
                ]
            ) {
                unchecked {
    ...
    function _addUniswapV3Leafs(
        ...
        for (uint256 i; i < token0.length; ++i) {
            ...
            if (
                !tokenToSpenderToApprovalInTree[token0[i]][getAddress(sourceChain, "uniV3Router")]
            ) {
                unchecked {
                    leafIndex++;
                }
            ...
            if (
                !tokenToSpenderToApprovalInTree[token1[i]][getAddress(sourceChain, "uniV3Router")]
            ) {
                unchecked {
                    leafIndex++;
                }
    ...
    function _addPendleMarketLeafs(
        ...
        for (uint256 i; i < possibleTokensIn.length; ++i) {
            if (
                ...
                !tokenToSpenderToApprovalInTree[possibleTokensIn[i]][
    ...
    function _addPendleLimitOrderLeafs(ManageLeaf[] memory leafs, address marketAddress) internal {
        ...
        if (
            !tokenToSpenderToApprovalInTree[yt][getAddress(sourceChain, "pendleLimitOrderRouter")]
        ) {
            unchecked {
                leafIndex++;
            }
            ...
            tokenToSpenderToApprovalInTree[yt][
                getAddress(sourceChain, "pendleLimitOrderRouter")
            ] = true;
        }

        if (
            !tokenToSpenderToApprovalInTree[pt][getAddress(sourceChain, "pendleLimitOrderRouter")]
        ) {
            unchecked {
                leafIndex++;
            }
            ...
            tokenToSpenderToApprovalInTree[pt][
                getAddress(sourceChain, "pendleLimitOrderRouter")
            ] = true;
        }

        if (
            !tokenToSpenderToApprovalInTree[sy][getAddress(sourceChain, "pendleLimitOrderRouter")]
        ) {
            unchecked {
                leafIndex++;
            }
            ...
            tokenToSpenderToApprovalInTree[sy][
                getAddress(sourceChain, "pendleLimitOrderRouter")
            ] = true;
        }
    ...
    function _addBalancerLeafs(ManageLeaf[] memory leafs, bytes32 poolId, address gauge) internal {
        ...
        for (uint256 i; i < tokens.length; i++) {
            if (
                ...
                !tokenToSpenderToApprovalInTree[address(tokens[i])][
    ...
    function _addMorphoBlueSupplyLeafs(ManageLeaf[] memory leafs, bytes32 marketId) internal {
        ...
        // Add approval leaf if not already added
        if (
            !tokenToSpenderToApprovalInTree[marketParams.loanToken][
    ...
    function _addMorphoBlueCollateralLeafs(ManageLeaf[] memory leafs, bytes32 marketId) internal {
        ...
        if (
            !tokenToSpenderToApprovalInTree[marketParams.collateralToken][
                getAddress(sourceChain, "morphoBlue")
            ]
        ) {
            unchecked {
    ...
    function _addLeafsForFeeClaiming(ManageLeaf[] memory leafs, ERC20[] memory feeAssets) internal {
        ...
        for (uint256 i; i < feeAssets.length; ++i) {
            if (
                !tokenToSpenderToApprovalInTree[address(feeAssets[i])][
    ...
    function _verifyDecoderImplementsLeafsFunctionSelectors(
        ...
        for (uint256 i; i < leafs.length; ++i) {
            ...
            (bool success, bytes memory returndata) = leafs[i].decoderAndSanitizer.staticcall(
